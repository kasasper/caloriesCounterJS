...
function addNewItem() {
    ...
    if (validate()) {
        ...
        let sizeElement = document.querySelector('select#size');
        let sizeTxt = sizeElement.options[sizeElement.selectedIndex].text.trim();
        let sizeArray = sizeTxt.split(' ');
        let newItem = {
...
function changeCalories() {
    ...
    if (pack === 'g')
        size = size / 100;
    target.querySelector('.counter-cal').innerText = String(Math.round(size * cal));
    sumCalories();
}
...
function saveGoal() {
    if (isValidNumber('goal')) {
        let userGoalCheck = settingsGoal.value;
